#ifndef EvaluateAtNodes_h
#define EvaluateAtNodes_h
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
class EvaluateAtNodes : public vtkm::worklet::WorkletMapField
{
  ...
  public:
    ...
    vtkm::FloatDefault dBr_dr=0, Br=0, dBz_dz=0;
    vtkm::Vec3f deltaB_rzp, B_dB_rzp;

  };

public:
  using ControlSignature = void(FieldIn Coords,
                                WholeArrayInOut Vec_B_RZP,
                                WholeArrayInOut Vec_dB_RZP,
                                WholeArrayInOut VecFieldRZP,
                                WholeArrayInOut dBr_dr,
                                WholeArrayInOut Br,
                                WholeArrayInOut dBz_dz,
                                ExecObject locator,
  ...
  EvaluateAtNodes(const XGCParameters& xgcParams)
  {
...
/*
  ...
  VTKM_EXEC void operator()(const vtkm::Id& idx,
                            const vtkm::Vec3f& ptRZ,
                            WholeArrayType<vtkm::Vec3f>& Vec_B_RZP,
                            WholeArrayType<vtkm::Vec3f>& Vec_dB_RZP,
                            WholeArrayType<vtkm::Vec3f>& VecFieldRZP,
                            WholeArrayType<vtkm::FloatDefault>& dBr_dr,
                            WholeArrayType<vtkm::FloatDefault>& Br,
                            WholeArrayType<vtkm::FloatDefault>& dBz_dz,
                            ...
                            const WholeArrayInType<vtkm::Vec3f>& B_RZP) const
  {
    ...
    ParticleInfo pInfo;
    for (vtkm::Id i = 0; i < this->NumPlanes; i++)
    {
      vtkm::Vec3f ptRPZ(ptRZ[0], phi, ptRZ[1]);
      vtkm::Vec3f resRPZ;

      if (!this->Evaluate(ptRPZ, pInfo, locator, cellSet, AsPhiFF, DAsPhiFF_RZP, Coeff_1D, Coeff_2D, B_RZP, resRPZ))
      {
        //std::cout<<"Fail: "<<ptRPZ<<std::endl;
        //std::cout<<"   "<<idx<<" planeIdx= "<<i<<" dPhi= "<<this->dPhi<<std::endl;
        //throw vtkm::cont::ErrorBadValue("Point evaluation failed!");
        resRPZ = vtkm::Vec3f(0,0,0);
        pInfo.dBr_dr = 0;
        pInfo.Br = 0;
        pInfo.dBz_dz = 0;
      }

      vtkm::Id outIdx = (i * this->NumNodes) + idx;
      VecFieldRZP.Set(outIdx, resRPZ);
      Vec_B_RZP.Set(outIdx, pInfo.B0_rzp);
      Vec_dB_RZP.Set(outIdx, pInfo.deltaB_rzp);
      dBr_dr.Set(outIdx, pInfo.dBr_dr);
      Br.Set(outIdx, pInfo.Br);
      dBz_dz.Set(outIdx, pInfo.dBz_dz);
                ...
                vtkm::Vec3f& res) const
  {
    ...
    */
    ...
    res = vec_rpz;
    pInfo.B0_rzp = B0_rzp;
    pInfo.B_dB_rzp = vec_rzp;
    pInfo.deltaB_rzp = deltaB_rzp;
    return true;
  }
