...
double operatorT = 0, rk4T = 0, evalT = 0, evalB0T = 0, evalB1T = 0, ffPtT = 0, deltaBT=0;
  double ptLocT = 0;

#ifdef TIME_STUFF
#define FT(x) internal3::FuncTimer tt(&x);
#else
#define FT(x)
#endif
#endif
...
class PoincareWorklet3 : public vtkm::worklet::WorkletMapField
{
  ...
  void PrintTimers() const
  {
    ...
    this->Print("operator()   :", internal3::operatorT, internal3::operatorT);
    this->Print(" rk4         :", internal3::rk4T, internal3::operatorT);
    this->Print(" eval        :", internal3::evalT, internal3::operatorT);
    this->Print(" deltaB      :", internal3::deltaBT, internal3::operatorT);
    this->Print(" ffPt        :", internal3::ffPtT, internal3::operatorT);
    this->Print(" evalB0      :", internal3::evalB0T, internal3::operatorT);
    this->Print(" evalB1      :", internal3::evalB1T, internal3::operatorT);
    this->Print(" ptLoc       :", internal3::ptLocT, internal3::operatorT);
#endif
...
public:
  ...
  using ExecutionSignature = void(InputIndex, _1, _2, _3, _4, _5, _6, _7, _8, _9, _10, _11, _12, _13, _14, _15, _16, _17, _18);
...
//    this->PrintTimers();
  ...
  template <typename LocatorRZType, typename LocatorBType, typename CellSetRZType, typename CellSetBType, typename PsiFieldType, typename BFieldType, typename AsFieldType, typename DAsFieldType, typename Coeff_1DType, typename Coeff_2DType, typename OutputType, typename OutputType2D, typename IdType>
                            ...
                            IdType punctureID) const
  {
    ...
    while (true)
    {
      ...
      if (!this->TakeRK4Step(particleRPZ,
                             locatorRZ, locatorB, cellSetRZ, cellSetB,
                             Psi, B_RZP, B_Curl_RZP, Curl_NB_RZP, GradPsi_RZP,
      ...
      //std::cout<<"    Step--> "<<particleRPZ<<std::endl;
...
*/
  ...
  template <typename LocatorRZType, typename LocatorBType, typename CellSetRZType, typename CellSetBType, typename PsiFieldType, typename BFieldType, typename AsFieldType, typename DAsFieldType, typename Coeff_1DType, typename Coeff_2DType>
  VTKM_EXEC
                   ...
                   vtkm::Vec3f& res) const
  {
    ...
    DBG("    ****** K1"<<std::endl);
    if (!this->Evaluate(p0, locatorRZ, locatorB, cellSetRZ, cellSetB, Psi, B_RZP, B_Curl_RZP, Curl_NB_RZP, GradPsi_RZP, AsPhiFF, DAsPhiFF_RZP, Coeff_1D, Coeff_2D, k1))
      return false;
    tmp.Pos = p0.Pos + k1*this->StepSize_2;
    //std::cout<<" K1: "<<p0.Pos<<" --> "<<tmp.Pos<<" k1= "<<k1<<std::endl;

    DBG("    ****** K2"<<std::endl);
    if (!this->Evaluate(tmp, locatorRZ, locatorB, cellSetRZ, cellSetB, Psi, B_RZP, B_Curl_RZP, Curl_NB_RZP, GradPsi_RZP, AsPhiFF, DAsPhiFF_RZP, Coeff_1D, Coeff_2D, k2))
      return false;
    tmp.Pos = p0.Pos + k2*this->StepSize_2;

    DBG("    ****** K3"<<std::endl);
    if (!this->Evaluate(tmp, locatorRZ, locatorB, cellSetRZ, cellSetB, Psi, B_RZP, B_Curl_RZP, Curl_NB_RZP, GradPsi_RZP, AsPhiFF, DAsPhiFF_RZP, Coeff_1D, Coeff_2D, k3))
      return false;
    tmp.Pos = p0.Pos + k3*this->StepSize;

    DBG("    ****** K4"<<std::endl);
    if (!this->Evaluate(tmp, locatorRZ, locatorB, cellSetRZ, cellSetB, Psi, B_RZP, B_Curl_RZP, Curl_NB_RZP, GradPsi_RZP, AsPhiFF, DAsPhiFF_RZP, Coeff_1D, Coeff_2D, k4))
      return false;
  ...
  EvalS4(const PortalType& sPortal,
         ...
         const vtkm::Vec3f& param) const
  {
    //return sPortal.Get(vId[0]);
    vtkm::VecVariable<vtkm::FloatDefault, 4> vals;
    vals.Append(sPortal.Get(vId[0]));
    vals.Append(sPortal.Get(vId[1]));
    vals.Append(sPortal.Get(vId[2]));
    vals.Append(sPortal.Get(vId[3]));

    vtkm::FloatDefault s;
    vtkm::exec::CellInterpolate(vals, param, vtkm::CellShapeTagQuad(), s);
    return s;
  }
  ...
  EvalV4(const PortalType& vPortal,
         ...
         const vtkm::Vec3f& param) const
  {
    //return vPortal.Get(vId[0]);
    //std::cout<<"    portalSize= "<<vPortal.GetNumberOfValues()<<std::endl;
    //std::cout<<"    ******** vid= "<<vId[0]<<" "<<vId[1]<<" "<<vId[2]<<std::endl;
    //std::cout<<"    ******** vec= "<<vPortal.Get(vId[0])<<" "<<vPortal.Get(vId[1])<<" "<<vPortal.Get(vId[2])<<std::endl;
    //std::cout<<"    ******** par= "<<param<<std::endl;

    vtkm::VecVariable<vtkm::Vec3f, 4> vals;
    vals.Append(vPortal.Get(vId[0]));
    vals.Append(vPortal.Get(vId[1]));
    vals.Append(vPortal.Get(vId[2]));
    vals.Append(vPortal.Get(vId[3]));

    vtkm::Vec3f res;
    vtkm::exec::CellInterpolate(vals, param, vtkm::CellShapeTagQuad(), res);
    return res;
  }
              ...
              vtkm::Vec<vtkm::Id, 4>& vIds) const
  {
    ...
    {
#ifndef VTKM_CUDA
      std::cout<<"PtLoc4: Point not found: "<<ptRZ<<std::endl;
#endif
      return false;
    }
...
DRP: field_following_pos2() i=             2
 ...
 DRP: field_following_pos2() x_dest=    2.819705204354387 -2.3230889173063193E-002
  ...
  template <typename LocatorBType, typename CellSetBType, typename FieldType, typename Coeff_1DType, typename Coeff_2DType>
  VTKM_EXEC
                            ...
                            vtkm::Vec3f& x_ff_rpz) const
  {
    ...
    */
    ...
    for (int i = 0; i < 2; i++)
    {
      this->EvalB(p0, locatorB, cellSetB, B0Field_RZP, k1);
      //k1 = this->GetB(p0, coeff_1D, coeff_2D);
      tmp.Pos = p0.Pos + k1*h_2;

      //k2 = this->GetB(tmp, coeff_1D, coeff_2D);
      this->EvalB(p0, locatorB, cellSetB, B0Field_RZP, k2);
      tmp.Pos = p0.Pos + k2*h_2;

      //k3 = this->GetB(tmp, coeff_1D, coeff_2D);
      this->EvalB(p0, locatorB, cellSetB, B0Field_RZP, k3);
      tmp.Pos = p0.Pos + k3*h;

      //k4 = this->GetB(tmp, coeff_1D, coeff_2D);
      this->EvalB(p0, locatorB, cellSetB, B0Field_RZP, k4);
  ...
  template <typename LocatorRZType, typename CellSetRZType, typename LocatorBType, typename CellSetBType, typename BFieldType, typename AsFieldType, typename DAsFieldType, typename Coeff_1DType, typename Coeff_2DType>
  VTKM_EXEC
                         ...
                         const Coeff_2DType& coeff_2D) const
  {
    ...
    this->CalcFieldFollowingPt({R,phiN,Z}, B0_rpz, Phi0, Phi1, locatorB, cellSetB, B0Field_RZP, coeff_1D, coeff_2D, ff_pt_rpz);
    ...
    this->PtLoc(x_ff_rzp, locatorRZ, cellSetRZ, x_ff_param, x_ff_vids);
  ...
  VTKM_EXEC
  void EvalB(const vtkm::Particle& pRPZ,
             const LocatorBType& locatorB,
             const CellSetBType& cellSetB,
             const PsiFieldType& Psi,
             const BFieldType& B_RZP,
             ...
             const BFieldType& Curl_NB_RZP,
             const BFieldType& GradPsi_RZP,
             vtkm::FloatDefault& resPsi,
             vtkm::Vec3f& resB_rzp,
             vtkm::Vec3f& resBCurl_rzp,
             vtkm::Vec3f& resNBCurl_rzp,
             vtkm::Vec3f& resGradPsi_rzp) const
  {
    vtkm::Vec3f ptRZ(pRPZ.Pos[0], pRPZ.Pos[2], 0);

    vtkm::Vec3f param;
    vtkm::Vec<vtkm::Id,4> vids;
    this->PtLoc4(ptRZ, locatorB, cellSetB, param, vids);

    resPsi = this->EvalS4(Psi, vids, param);
    resB_rzp = this->EvalV4(B_RZP, vids, param);
    resBCurl_rzp = this->EvalV4(B_Curl_RZP, vids, param);
    resNBCurl_rzp = this->EvalV4(Curl_NB_RZP, vids, param);
    resGradPsi_rzp = this->EvalV4(GradPsi_RZP, vids, param);
  }

  template <typename LocatorBType, typename CellSetBType, typename BFieldType>
  VTKM_EXEC
  void EvalB(const vtkm::Particle& pRPZ,
             const LocatorBType& locatorB,
             const CellSetBType& cellSetB,
             const BFieldType& B_RZP,
             vtkm::Vec3f& resB_rzp) const
  {
                     ...
                     vtkm::Vec3f& res) const
  {
    ...
    vtkm::FloatDefault ePsi;
    vtkm::Vec3f eB0_rzp, eCurlB_rzp, eCurlNB_rzp, eGradPsi_rzp;
    this->EvalB(pRPZ, locatorB, cellSetB,
                Psi, B_RZP, B_Curl_RZP, Curl_NB_RZP, GradPsi_RZP,
                ePsi, eB0_rzp, eCurlB_rzp, eCurlNB_rzp, eGradPsi_rzp);
    pRPZ.Psi = ePsi;
    pRPZ.B0_rzp = eB0_rzp;
    pRPZ.curlB_rzp = eCurlB_rzp;
    pRPZ.curl_nb_rzp = eCurlNB_rzp;
    pRPZ.gradPsi_rzp = eGradPsi_rzp;
    ...
    std::cout<<"ptRPZ= "<<pRPZ.Pos<<std::endl;
    std::cout<<"PSI: "<<psi-ePsi<<std::endl;
    std::cout<<"B0: "<<B0_rzp<<" "<<eB0_rzp<<" "<<vtkm::Magnitude(B0_rzp-eB0_rzp)<<std::endl;
    std::cout<<"curlB0: "<<curlB_rzp<<" "<<eCurlB_rzp<<" "<<vtkm::Magnitude(curlB_rzp-eCurlB_rzp)<<std::endl;
    std::cout<<"curlNB0: "<<curl_nb_rzp<<" "<<eCurlNB_rzp<<" "<<vtkm::Magnitude(curl_nb_rzp-eCurlNB_rzp)<<std::endl;
    std::cout<<"gradPsi: "<<GRADPSI_rzp<<" "<<eGradPsi_rzp<<" "<<vtkm::Magnitude(GRADPSI_rzp-eGradPsi_rzp)<<std::endl;
    */
    ...
    deltaB_rzp = this->CalcDeltaB(pRPZ, B0_rzp, GRADPSI_rzp, curl_nb_rzp,
                                  locatorRZ, cellSetRZ, locatorB, cellSetB,
                                  B_RZP, AsPhiFF, DAsPhiFF_RZP, Coeff_1D, Coeff_2D);
    deltaB_rzp[2] /= R;
    B0_rzp[2] /= R;
  ...
  bool Evaluate(vtkm::Particle& pRPZ,
                ...
                vtkm::Vec3f& res) const
  {
    return this->HighOrderEval(pRPZ,
                               locatorRZ, locatorB, cellSetRZ, cellSetB,
                               Psi, B_RZP, B_Curl_RZP, Curl_NB_RZP, GradPsi_RZP,
                               AsPhiFF, DAsPhiFF_RZP, Coeff_1D, Coeff_2D, res);
  }
