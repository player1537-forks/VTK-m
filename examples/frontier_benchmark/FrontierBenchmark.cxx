...
{
  struct stat info;
  if (stat(options.CameraMode.c_str(), &info) != 0)
  {
    std::stringstream msg;
    msg << "ERROR: directory " << options.CameraMode << " does not exist.";
...
void RunBenchmark(const BenchmarkOptions& options)
{
  ...
  std::string imgFile = GetImageName("perlin_static", options, mpiTopology);

  vtkm::cont::PartitionedDataSet pds(dataSet);
  globalBounds = pds.GetGlobalBounds();
