...
struct VTKM_RENDERING_EXPORT PayloadImage
{
    ...
    {
      return static_cast<int>(this->Depths.size());
    }
              ...
              int height)
    {
      ...
      std::copy(payload_buffer,
                payload_buffer + size * this->PayloadBytes,
                &this->Payloads[0]);
                    ...
                    const vtkm::Bounds &sub_region)
    {
      ...
      for(int y = start_y; y < end_y; ++y)
      {
        ...
        std::copy(&image.Depths[copy_from],
                  &image.Depths[copy_from] + s_dx,
                  &this->Depths[copy_to]);
      }
    ...
    void SubsetTo(PayloadImage &image) const
    {
      ...
      for(int y = 0; y < s_dy; ++y)
      {
        ...
        std::copy(&this->Depths[copy_from],
                  &this->Depths[copy_from] + s_dx,
                  &image.Depths[copy_to]);
      }
    }
