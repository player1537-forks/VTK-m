...
void ZBufferBlend(std::vector<vtkh::Image> &images)
{
  ...
  for(int i = 0; i < image_pixels; ++i)
  {
    ...
    for(int j = 1; j < num_images; ++j)
    {
      ...
      const unsigned int opacity = 255 - alpha;
      pixel.Color[0]
        += static_cast<unsigned char>(opacity * pixels[index + j].Color[0] / 255);
      pixel.Color[1]
        += static_cast<unsigned char>(opacity * pixels[index + j].Color[1] / 255);
      pixel.Color[2]
        += static_cast<unsigned char>(opacity * pixels[index + j].Color[2] / 255);
      pixel.Color[3]
        += static_cast<unsigned char>(opacity * pixels[index + j].Color[3] / 255);
