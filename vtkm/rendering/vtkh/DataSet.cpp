...
DataSet::IsPointMesh() const
{
  ...
  for(size_t i = 0; i < num_domains; ++i)
  {
    ...
    bool single_type = vtkh::VTKMDataSetInfo::IsSingleCellShape(dom.GetCellSet(), shape_type);
...
DataSet::IsUnstructured() const
{
  ...
  for(size_t i = 0; i < num_domains; ++i)
  {
    const vtkm::cont::DataSet &dom = m_domains[i];
    int dims;
    is_unstructured = !vtkh::VTKMDataSetInfo::IsStructured(dom, dims) && is_unstructured;
...
DataSet::IsStructured(int &topological_dims) const
{
  ...
  for(size_t i = 0; i < num_domains; ++i)
  {
    const vtkm::cont::DataSet &dom = m_domains[i];
    int dims;
    is_structured = vtkh::VTKMDataSetInfo::IsStructured(dom, dims) && is_structured;
...
DataSet::GetFieldAssociation(const std::string field_name, bool &valid_field) const
{
  ...
  {
    throw vtkm::cont::ErrorBadValue("Get association: unknown association");
  }
  return assoc;
}
*/
