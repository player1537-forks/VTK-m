...
bool GlobalAgreement(bool local)
{
  ...
  MPI_Allreduce((void*)(&local_boolean), (void*)(&global_boolean), 1, MPI_INT, MPI_SUM, mpi_comm);
...
bool IsEmpty(const vtkm::cont::PartitionedDataSet& pds)
{
...
bool GlobalHasOnePartition(const vtkm::cont::PartitionedDataSet& pds)
{
  bool one = pds.GetNumberOfPartitions() == 1;
  return detail_utils::GlobalAgreement(one);
}

bool GlobalHasField(const vtkm::cont::PartitionedDataSet& pds, const std::string& fieldName)
{
  bool haveLocal = true;
  for (const auto& ds : pds.GetPartitions())
...
bool GlobalIsEmpty(const vtkm::cont::PartitionedDataSet& pds)
{
  return detail_utils::GlobalAgreement(vtkh::IsEmpty(pds));
}

bool IsPointMesh(const vtkm::cont::PartitionedDataSet& pds)
{
